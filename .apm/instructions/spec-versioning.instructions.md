...
- Once a spec is `🟣 Planned` or later, **any substantive change** to it must bump `**Version:**`
  and add a `## Changelog` row. Substantive = changes to overview, user stories, acceptance-criteria
  *text*, edge cases, or tech-design contracts. **Not** substantive (no bump): advancing status,
  editing the changelog, ticking ACs, or appending the close-out `## Implementation & Verification`
  section — so the normal implement → verify → close-out flow needs no bumps.
...
- **Deprecation (drop, don't rewrite):** when a change makes a spec's feature no longer exist,
  ...
  add a changelog row citing the obsoleting `SPEC-N`, append a short `## Deprecation` note, and
  **keep the file as a tombstone** (never delete it). Deprecated specs are frozen: the AC-closeout
  and status-floor checks no longer apply to them.
